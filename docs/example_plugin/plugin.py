...
class LibertySystemOutPlugin(DataSourcePlugin):
    ...
    def get_description(self) -> str:
        """Return detailed description of the plugin in HTML format."""
        return """
...
File size limits: 100 bytes minimum, 500MB maximum
    ...
    def generate_html(self, data: Dict[str, Any]) -> str:
        ...
        Note: The section header and documentation wrapper are now generated centrally
        by JavacoreSet.generate_plugin_section_header() using the plugin's display name
        and description. This method should only return the content HTML without the
        ...
        Returns:
            HTML string containing only the section content (tables, charts, etc.)
        """
        # Return empty string if no data
        if not data or data.get('total_files', 0) == 0:
            return ''
        
        # Build HTML content - section header is now generated by JavacoreSet
